  ...
  <div>
    <div class="col-xs-24 col-md-11">
      ...
      <div class="form-group">
        ...
        <autocomplete-site :prompt="spot.site.name_and_address" what="sites_in_organisation"
          v-on:picked="setSiteId"></autocomplete-site>
      </div>
      <div class="form-group">
        <div class="row">
          <div class="col-xs-8">
            ...
            <div class="switch">
              <input type="radio" class="switch-input" v-model="spot.type" value="Spots::Audio" id="spot-audio"
                checked />
      ...
      <div class="form-group">
        <div class="row">
          <div class="col-xs-8">
            ...
            <div class="switch">
              <input type="radio" class="switch-input" v-model="spot.real_time_usage" value="true" id="realtime"
                checked />
          ...
          <div class="col-xs-8" style="text-align: right;">
            ...
            <div v-if="spot.is_screen == true" class="switch" style="float: right;">
              <input type="radio" class="switch-input" v-model="spot.native_portrait" value="true" id="native"
                checked />
      ...
      <div class="form-group">
        ...
        <input type="radio" name='dhcp' @change="togglePhysicalData(false)" :checked="!spot.network_json"> {{
          $t('box.dhcp') }}
        <input type="radio" name='dhcp' @change="togglePhysicalData(true)" :checked="spot.network_json"> {{
          $t('box.fixed_ips') }}
        &nbsp;
        ...
        <div v-if="spot.network_json">
          ...
          <div class="well">
            ...
            <input type="text" name="network_json" v-model="spot.network_json.network_proxy_password"
              class="form-control">
          </div>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="switch">
          <input type="radio" class="switch-input" v-model="spot.force_original" value="true" id="spot-original"
            checked />
      ...
      <div class="form-group">
        ...
        <input type="radio" name="geometry" @change="toggleGeometryData(false)" :checked="!spot.geometry_json"> {{
          $t('box.geometry.auto') }}
        <input type="radio" name="geometry" @change="toggleGeometryData(true)" :checked="spot.geometry_json"
          style="margin-left: 20px;"> {{ $t('box.geometry.custom') }}
      ...
      <div class="form-group">
        ...
        <input type="radio" name='storage_kind' v-model="storage_kind" value="false" style="margin-bottom: 10px"
          checked>
        Non renseigné
        <br>
        <input type="radio" name='storage_kind' v-model="storage_kind" value="percent">
        <input type="number" name="storage" min="0" max="100" style="margin-bottom: 10px"
          v-model="storage_value_percent">
        %
        <br>
    ...
    <div class="col-xs-24 col-md-11 pull-right">
      ...
      <div class="form-group">
        ...
        <div v-if="spot.device.id" class="row">
          ...
          <div class="col-sm-6">
            <a href="#" @click="modalSav" class="btn btn-warning" style="height: 40px; padding-top: 10px">{{
              $t('device.reallocate') }}</a>
            <modal-sav ref="modalSav"></modal-sav>
          </div>
        </div>
        <div v-else>
          <select v-model="spot.device_id" class="form-control col-sm-8" @change="showDeviceDetails">
            <option v-for="device in spot.devices" :key="device.id" :value="device.id">{{ device.id }} - {{
              device.serial }}</option>
          </select>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group" style="width: 40%;float: left">
        <input type="checkbox" value="true" @change="setNtp" :checked="spot.ntp_server != ''"
          style="float: left; margin: 4px 10px auto auto;"><b>Forcer l'usage du NTP</b>
      ...
      <div class="form-group" style="width: 50%;float: right">
        <b>Teamviewer ID</b>
        <br />
        <p v-if="spot.device.teamviewer_id != ''">{{ spot.device.teamviewer_id }}</p>
        <p v-else>Unknown</p>
      </div>
      <div class="form-group">
      ...
      <div class="form-group" v-if="spot.soc">
        <label v-if="spot.pairing_code">Code d'appairage: <br /><span
            style="font-size: 30px">{{ spot.pairing_code }}</span></label>
        <label v-else>Pas de code d'appairage . <a style="cursor: pointer;"
            @click="setPairingCode()">Générer</a></label>
      </div>
      ...
      <a v-if="spot.id != null && spot.active" href="#" class="btn btn-danger" @click.prevent="destroy">{{ $t('delete')
        }}</a>
      <a v-if="spot.id != null" href="#" @click="forceSync" class="btn btn-danger pull-right"
        style="margin-right: 10px;">Générer le catalogue</a>
      <a class="btn btn-danger pull-right" v-if="cache_btn" @click="generateCache"
        style="cursor: pointer; margin-right: 10px">Générer le cache</a>
    </div>
