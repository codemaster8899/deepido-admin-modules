        ...
        <div class="row styled-form">
          <div class="col-xs-24">
            <p style="text-align: right">
              <router-link :to="'/playlists/' + playlist.id + '/history'" style="float: left;">&laquo;&nbsp;Historique
                des modifications</router-link>
              <b v-if="tracks_mode.name == 'all'">Etat de la playlist au {{ tracks_mode.edited_at }}</b>
              <b v-if="tracks_mode.name == 'added'">Titres ajoutés à la playlist le {{ tracks_mode.edited_at }} par
                {{ tracks_mode.edited_by }}</b>
              <b v-if="tracks_mode.name == 'removed'">Titres retirés de la playlist le {{ tracks_mode.edited_at }} par
                {{ tracks_mode.edited_by }}</b>
            </p>
